...
import { ChakraBaseProvider, cookieStorageManagerSSR } from "@chakra-ui/react";
import type {
  ActionFunctionArgs,
  LinksFunction,
  LoaderFunctionArgs,
...
function InsideGlobal() {
  const { colorMode } = useTypedLoaderData<typeof loader>();
    ...
    <ChakraBaseProvider
      ...
      colorModeManager={cookieStorageManagerSSR(
        typeof document !== "undefined"
          ? document.cookie
          : `chakra-ui-color-mode=${colorMode}`
      )}
    >
      <Layout>
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const bedrock =
    getCookieWithoutDocument("version", request.headers.get("Cookie") ?? "") ===
    "bedrock";
