...
import { fetch, type MetaFunction, type LoaderArgs, defer } from "@remix-run/node"
...
export async function loader({ params, request }: LoaderArgs) {
    ...
    return defer({ server, data, checks })
};
...
export default function $server() {
    ...
    const lastServer = useRef({})
    const lastChecks = useRef({})
    const { server, data, checks } = useLoaderData() || { server: lastServer.current, checks: lastChecks.current }
    useEffect(() => {
        if (server) lastServer.current = server
        if (checks) lastChecks.current = checks
    }, [server, checks])
