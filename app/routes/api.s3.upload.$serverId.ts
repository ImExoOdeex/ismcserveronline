...
export async function action({ request, params }: ActionFunctionArgs) {
    ...
    try {
        ...
        invariant(!Number.isNaN(serverId), "Invalid server id");
        ...
        const form = await parseMultipartFormData(
            request,
            async ({ data, name, filename, contentType }) => {
        ...
        const url =
            (form.get("banner")?.toString() as string) ||
            (form.get("background")?.toString() as string);
