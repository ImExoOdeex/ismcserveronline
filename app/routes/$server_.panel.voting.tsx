...
import { Badge, Button, Code, Divider, Flex, FormControl, FormLabel, Heading, Input, Text, VStack } from "@chakra-ui/react";
...
export async function action({ request, params }: ActionFunctionArgs) {
	...
	switch (intent) {
		case "update": {
			try {
				const usingVotifier = form.get("usingVotifier") ? form.get("usingVotifier") === "true" : undefined;
				...
				const votifierPort = form.get("votifierPort")?.toString()
					? parseInt(form.get("votifierPort")?.toString() ?? "")
					: undefined;
...
export default function ServerPanel() {
	...
	const toggleFetcher = useDebouncedFetcherCallback((data) => {
		progressBar.done();
	});
	const hostFetcher = useDebouncedFetcherCallback((data) => {
		progressBar.startAndDone();
	});
	const portFetcher = useDebouncedFetcherCallback((data) => {
		progressBar.startAndDone();
	});
	const tokenFetcher = useDebouncedFetcherCallback((data) => {
		progressBar.startAndDone();
	});

	const [usingVotifier, setUsingVotifier] = useState(data.using_votifier ?? false);

	const [votifierHost, setVotifierHost] = useState(data.votifier_host ?? removePortFromHost(data.server));
		...
		<Flex gap={10} w="100%" flexDir={"column"}>
			...
			<Flex gap={4} flexDir={"column"}>
				...
				<Flex gap={4} flexDir={"column"}>
					...
					<Text>
						While using our plugin you have to install it on your server and put your server's token in the config
						file. After you've done that, players can run <Code>/vote reward</Code> command in-game to recieve their
						reward. You can configure rewards while holding an item you want to give to the player and running{" "}
						<Code>/vote additem</Code> command.
					</Text>
				</Flex>
				...
				<Flex gap={4} flexDir={"column"}>
					<Flex gap={4} flexDir={"row"} alignItems={"center"} w={"100%"} justifyContent={"space-between"}>
						<Flex gap={2} flexDir={"column"} alignItems={"flex-start"}>
					...
					<Text>
						Votifier is a plugin that allows your server to receive votes from various server list websites.
						<br />
						To use Votifier, you need to install the plugin on your server and get it's token and (optionally) port.
					</Text>

					<FormControl isInvalid={!votifierToken && (votifierToken.length < 1 || votifierPort > 250)}>
					...
					<FormControl isInvalid={!!votifierHost && (votifierHost.length < 1 || votifierHost.length > 250)}>
					...
					<FormControl isInvalid={!isNaN(votifierPort) && !!votifierPort && (votifierPort < 1 || votifierPort > 65535)}>
						...
						<Input
							...
							value={votifierPort}
							onChange={(e) => {
								setVotifierPort(parseInt(e.target.value));
								portFetcher.submit(
									{
										intent: "update",
										votifierPort: parseInt(e.target.value)
									},
									{
										method: "PATCH",
										debounceTimeout: 250
