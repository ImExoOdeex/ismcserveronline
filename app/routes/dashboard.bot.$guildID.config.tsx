...
export async function loader({ params, request }: LoaderFunctionArgs) {
    ...
    const colorPromises = Promise.all(
        [config.online_color, config.offline_color].map((color: number) => {
            return fetch(
                `https://www.thecolorapi.com/id?hex=${decimalToHex(color).replace("#", "")}`,
                {
                    headers: {
...
export async function action({ request, params }: ActionFunctionArgs) {
    ...
    getUser(request).then((user) => {
        if (user) {
            ...
            sendActionWebhook(
                user,
                `modify config. online: ${formData.get("onlineColor")}, offline: ${formData.get(
                    "offlineColor"
                )}`,
...
export default function Config() {
    ...
    return (
        <>
            ...
            <Text color={"textSec"}>
                To edit status colors for the bot's embed responses, click on the "Color Fill" icon
                located on the bottom right corner of the color box. This will open up the color
                picker. Changes will be saved automatically.
            </Text>
        </>
    );
}
