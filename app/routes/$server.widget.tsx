...
import { getWidgetFonts } from "@/.server/modules/widgets";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
    ...
    let png: string | Buffer | null = await cache.get(request.url);
    if (png) png = Buffer.from(png, "base64");
    ...
    if (!png) {
        ...
        const rounded =
            url.searchParams.has("rounded") && url.searchParams.get("rounded") !== "false";
                        ...
                        >
                            ...
                            >
                                <h2 style={{ fontSize: 48, opacity: 0.6, margin: 0 }}>
                                    {data.players.online}/{data.players.max} players are playing
                                    right now!
                                </h2>
                            </div>
                        </div>
        ...
        const fonts = await getWidgetFonts();
        ...
        png = image.asPng();
        await cache.set(request.url, png.toString("base64"), 3600);
    }
