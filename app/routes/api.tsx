import { Badge, Box, Code, Flex, HStack, Heading, Icon, Link, ListItem, OrderedList, Stack, Text, Tooltip, VStack, useClipboard, useColorModeValue } from "@chakra-ui/react";
...
export async function loader() {
    ...
    return json({ sampleToken }, {
        headers: [["Cache-Control", "max-age=0"], ["Set-cookie", "hasSeenNew=true"]]
    })
};
...
export default function Api() {
    ...
    const { sampleToken } = useLoaderData<{ sampleToken: string }>() || { sampleToken: lastSampleToken.current }
    useEffect(() => {
            ...
            <VStack maxW={'1200px'} w='100%' align={'start'} mx='auto' px={4} spacing={{ base: 10, md: "125px" }} mt={10}>
                ...
                <VStack w='100%' align={'start'} spacing={16}>
                    <VStack w='100%' alignItems={'start'} spacing={2}>
                        ...
                        <Text>To use the api, enter your generated token into <Badge>Autorization</Badge> header, like shown on example below. It is important to note that API tokens should be kept secure and not shared with unauthorized users.</Text>
