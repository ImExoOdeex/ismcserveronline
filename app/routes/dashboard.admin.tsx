            ...
            <Flex w="100%" justify={"flex-end"}>
                ...
                <flushFetcher.Form method="DELETE">
                    <Button
                        variant={"brand"}
                        type="submit"
                        isLoading={flushFetcher.state !== "idle"}
                    >
                        Flush Cache
                    </Button>
                </flushFetcher.Form>
                                                        ...
                                                        <Th>
                                                            {
                                                                (
                                                                    server.Server
                                                                        .players as unknown as ServerModel.Players<false>
                                                                )?.online
                                                            }
                                                        </Th>
                                                        <Th>
                                            ...
                                            <Tbody>
                                                {stats.verifiedServers.map((server) => (
                                                    <Tr key={"verified-server-" + server.id}>
                                                        ...
                                                        <Th>{server.server}</Th>
                                                        <Th>
                                                            {server.Owner?.nick ?? "Not verified"}
                                                        </Th>
                                                        <Th>
                                                            <ChakraLink
                                                                href={server.banner ?? ""}
            ...
            {bot && (
                <Flex flexDir={"column"} gap={4}>
                    ...
                    <SimpleGrid columns={2} gap={4}>
                        {Object.entries(copyObjectWithoutKeys(bot, ["biggestGuilds"])).map(
                            ([key, value]: [any, any]) => (
                                <Flex
                                    key={key}
                                    flexDir={"column"}
