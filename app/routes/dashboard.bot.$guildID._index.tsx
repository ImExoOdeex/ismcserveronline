    ...
    if (edition) {
        if (
            ...
            return json(
                {
                    message:
                        "Address must be between 3 and 100 characters and must be valid domain/IP address.",
                    success: false
                },
                { status: 400 }
            );
    ...
    return (
        <VStack w="100%" align={"start"} spacing={10}>
            ...
            <livecheckFetcher.Form method="POST" style={{ width: "100%" }}>
                <VStack w="100%" align={"start"} spacing={5}>
                    <VStack align={"start"} spacing={1}>
                        ...
                        {livecheck && (
                            <Code fontSize={"xs"}>
                                Next check at{" "}
                                {new Date(livecheck.next_check_date).toLocaleTimeString()}
                            </Code>
                        )}
                    </VStack>
                            ...
                            <VStack align={"start"} spacing={0} w={{ base: "100%", md: "lg" }}>
                                ...
                                {isEditing ? (
                                    <Select
                                        ...
                                        defaultValue={{
                                            label: `#${
                                                channels.find((c) => c.id === livecheck.channel_id)
                                                    ?.name
                                            }`,
                    ...
                    {livecheck && (
                        <Text fontSize={"xs"}>Data refreshes automatically every 30 seconds.</Text>
                    )}
                ...
                <Text color={"textSec"}>
                    Livecheck checks Minecraft's server status every 30 seconds and updates the
                    message in real-time if there are any changes to the player count or server
                    status. To set it up, enter the server address, select the correct Minecraft
                    edition, and choose the appropriate channel to display the server status.
                </Text>
