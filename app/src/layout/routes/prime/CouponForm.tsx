...
export default function CouponForm({ addedCoupon, setAddedCoupon, color, serverId }: Props) {
	...
	return (
		<Flex flexDir={"column"} alignItems={"flex-end"} justifyContent={"flex-end"}>
			{hidden ? (
				<Button variant={"unstyled"} color={color} onClick={() => setHidden(false)} fontWeight={500}>
					Have a coupon?
				</Button>
			) : addedCoupon ? (
				<Flex flexDir={"column"} gap={1} justifyContent={"flex-end"} alignItems={"flex-end"}>
					...
					<Flex flexDir={"column"} alignItems={"flex-end"}>
						...
						<Text fontSize={"lg"} fontWeight={"semibold"}>
							{addedCoupon.duration === "forever" ? "Forever" : addedCoupon.duration_in_months + " months"}
						</Text>
					</Flex>
				</Flex>
			) : (
				<FormControl maxW={"300px"}>
					...
					{!serverId && (
						<Text fontWeight={500} color={colorMode === "light" ? "orangered" : "orange"} fontSize={"sm"}>
