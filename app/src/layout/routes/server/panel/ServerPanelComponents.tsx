...
export const StatBox = memo(function StatBox({
    ...
    value,
    helper
}: { title: string; value: number; helper?: number }) {
        ...
        >
            <Stat>
                ...
                {(helper === 0 || helper) && (
                    <Tooltip
                        label={
                            "Percentage shows the increase in this month compared to the last month."
                        }
                        hasArrow
                    >
                ...
                <AlertDialogOverlay>
                    <AlertDialogContent bg="bg">
                        ...
                        <AlertDialogBody display={"flex"} flexDir={"column"} gap={4} pb={4}>
                            The recommended aspect ratio for the banner is 21:9. Below you can
                            download the template for the banner.
                            <Image src="/banner-template.png" alt="Banner template" />
                            <HStack w="100%">
                                <Button
                                    as="a"
                                    href="/banner-template.png"
                                    download
                                    w="100%"
                                    rightIcon={<PiFilePngBold />}
                                >
                                    Download PNG
                                </Button>
                                <Button
                                    as="a"
                                    href="/banner-template.psd"
                                    download
                                    w="100%"
                                    rightIcon={<SiAdobephotoshop />}
                                >
                                    Download PSD
                                </Button>
                            </HStack>
...
export const Tags = memo(function Tags({
    ...
    const addFetcher = useFetcherCallback<typeof action>((data) => {
        startAndDone();
        setSubmitting((prev) => prev.filter((id) => id !== (data as any).tag.name));
    });
    ...
    return (
        <>
            <Flex flexDir="column">
                ...
                <Text color={"textSec"}>
                    Add tags to your server. Tags are used to help users find your server. Swipe a
                    tag to remove.
                </Text>
            </Flex>
...
export const ServerTag = memo(function ServerTag({
    ...
    }, [deleteFetcher.submit, serverId, setTags, tag]);
