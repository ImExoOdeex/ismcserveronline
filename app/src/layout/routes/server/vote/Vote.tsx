...
export default memo(function Vote() {
  const { mcNick, user } = useRootData();
  let { vote } = useAnimationLoaderData<typeof loader>();
  ...
  const fetcher = useFetcherCallback<typeof action>((data) => {
    if (!data.success) {
      toast({
        title: "Vote failed",
        description:
          (fetcher.data as any)?.message || "An error occurred while voting",
        status: "error",
        isClosable: true,
      });
    }
  ...
  const [nick, setNick] = useState(mcNick);
  ...
  return (
    ...
    >
      <Flex flexDir={"column"} gap={1.5}>
        ...
        {vote && (
          <Text color="textSec">
            You will be able to vote again in{" "}
            {dayjs(vote.created_at).add(12, "hours").fromNow(true)}
          </Text>
        )}
      </Flex>
      ...
      >
        ...
        <Button
          ...
          isDisabled={
            vote !== null || secondsToVote !== 0 || fetcher.data?.success
          }
          w={"auto"}
        >
