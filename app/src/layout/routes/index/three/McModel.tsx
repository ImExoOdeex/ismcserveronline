...
export default function McModel() {
	...
	useEffect(() => {
		...
		if (container) {
			...
			const initialCameraPosition = new THREE.Vector3(-40 * Math.sin(0.2 * Math.PI), 10, -20 * Math.cos(0.2 * Math.PI));

			// 640 -> 240
			// 8   -> 6
			const scale = scH * 0.03 + 4.8;
			const camera = new THREE.OrthographicCamera(-scale, scale, scale, -scale, 0.001, 500000);
