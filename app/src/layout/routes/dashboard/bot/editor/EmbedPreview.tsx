...
export default function EmbedPreview({ message }: Props) {
    ...
    const thumbnail = usePlaceholdersReplaced(
        message.embed?.thumbnail?.url ?? "",
        placeholders[message.type]
    );
    const image = usePlaceholdersReplaced(
    ...
    return (
        ...
        >
            ...
            {/* image */}
            {message.embed?.image?.url && (
                <Image maxW={"400px"} w="100%" rounded={"lg"} mt={4} src={image} alt={image} />
            )}
