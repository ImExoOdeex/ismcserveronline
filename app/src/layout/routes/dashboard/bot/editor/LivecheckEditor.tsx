...
export default memo(function LivecheckEditor({ channels, messages, roles }: Props) {
	const [onlineMessage, setOnlineMessage] = useState<DiscordMessage>(messages[0]["ONLINE"]);
	const [offlineMessage, setOfflineMessage] = useState<DiscordMessage>(messages[0]["OFFLINE"]);
	...
	const [status, setStatus] = useState<(typeof statuses)[number]>("Online");
				...
				<HStack spacing={1}>
					{statuses.map((s) => (
						<Button
							...
							rounded={"lg"}
							_hover={{
								bg: status === s ? "rgba(98, 42, 167, 0.05)" : "rgba(98, 42, 167, 0.2)"
							}}
							_active={{
								bg: status === s ? "rgba(98, 42, 167, 0.1)" : "rgba(98, 42, 167, 0.3)"
							}}
							pos={"relative"}
						>
							{s}
