...
export default function StatusColor({
    ...
    const [color, setColor] = useState<string>(
        `#${configColorDecimal === 0 ? "000000" : configColorDecimal.toString(16)}`
    );
    ...
    const fetcher = useDebouncedFetcherCallback((_data) => {
        startAndDone();
    });
                ...
                <VStack w="100%" align={"start"}>
                    ...
                    >
                        {colorName ? (
                            ...
                            >
                                <Await resolve={colorPromises}>
                                    {(data) => <Text>{data[type === "online" ? 0 : 1]}</Text>}
                                </Await>
                            </Suspense>
                        )}
                    </Heading>
