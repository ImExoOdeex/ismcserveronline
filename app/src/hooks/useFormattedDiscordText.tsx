...
function processLine(line: string, mentionBg: string, data?: FormatData) {
    ...
    function processWord(word: string, index: number, isFirstInLine: boolean) {
        ...
        if (word.startsWith(">") && isFirstInLine)
            ...
            words.push(<Text as="b">{word?.replaceAll("**", "")}</Text>);
        else if (word.match(/__([^_]+?)__/g))
            words.push(<Text textDecor={"underline"}>{word?.replaceAll("__", "")}</Text>);
        else if (word.match(/\*([^*\n]+?)\*/g))
            words.push(<Text as="em">{word?.replaceAll("*", "")}</Text>);
        else if (word.match(/\_([^_\n]+?)\_/g))
            words.push(<Text as="em">{word?.replaceAll("_", "")}</Text>);
        else if (word.match(/~~([^~]+?)~~/g))
            words.push(<Text as="s">{word?.replaceAll("~~", "")}</Text>);
        else if (word.match(/\|\|([^|\n]+?)\|\|/g))
            words.push(
            ...
            words.push(
                <Image
                    src={`https://cdn.discordapp.com/emojis/${word
                        ?.split(":")[2]
                        ?.replaceAll(">", "")}.webp`}
                    alt={"previewImage"}
                    objectFit={"cover"}
    ...
    return useMemo(() => {
        ...
        for (const placeholder of placeholders) {
            newText = newText.replaceAll(
                ...
                placeholder.replaceWith ?? placeholder.name
            );
        }
